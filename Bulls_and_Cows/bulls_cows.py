...
def generate_secret_number():
    ...
    while len(random_number) < 4:
        digit = random.randint(1, 9)  # Generate a random digit
        if digit not in used_digits:  # Check if the digit has already been used
            random_number += str(digit)  # Add the digit to the result
            used_digits.add(digit)  # Add the digit to the set of used digits
...
def check_bulls_and_cows(secret_number, user_guess):
    ...
    for digit in range(4):
        if user_guess[digit] == secret_number[digit]:
            bulls += 1  # Right number in the right place
        elif user_guess[digit] in secret_number:
            cows += 1   # Right number in the wrong place
...
def play_bulls_and_cows():
    ...
    while not guessed_correctly:
        ...
        if not validate_user_input(user_guess):
            continue    # If the input is not valid, ask the player for another guess
